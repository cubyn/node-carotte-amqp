...
function Carotte(config) {
  ...
  carotte.publish = function publish(qualifier, options, payload = {}) {
    ...
    options = {
      headers: {}, context: {}, ...options, ...parseQualifier(qualifier),
    };
      ...
      .then((routingKey) => {
        ...
        const buffer = options.isContentBuffer
          ...
          : Buffer.from(JSON.stringify({
            data: payload,
            context: { ...options.context, transactionStack: getTransactionStack(options.context) },
          }));
  ...
  carotte.parallelWithFullResponse = function parallelWithFullResponse(qualifier, options, payload, callback) {
    if (arguments.length === 3) {
      callback = payload;
      payload = options;
      options = {};
  ...
  /**
     ...
     * @return {promise} return a new queue
     */
  carotte.subscribe = function subscribe(qualifier, options, handler, meta, logger) {
    let chan;
  ...
  /**
     ...
     */
  carotte.handleRetry = function handleRetry(qualifier, options, meta = {}, headers, context, message) {
    return (error) => {
